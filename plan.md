...
The app is a Flutter golf league manager at v0.0.1 with a basic skeleton: 3 models (Course, Hole, League), fake repositories, Riverpod providers, a single GoRouter route, and minimal UI (HomeScreen showing first course scorecard + league name). The requirements call for a full league home, interactive scorecard with golf indicators/skins/handicaps, round management, light/dark theming, strong linting, and 95% test coverage. This plan builds it out UI-first with fake data so screens are functional immediately.

Routes are added incrementally with each phase so the app is navigable and testable at every step.
...
- **Create:** `lib/round/presentation/score_entry_screen.dart` — `ConsumerStatefulWidget` managing local score edits. Editable Scorecard + bottom hole navigation bar + Save action in AppBar.
...
- **Create:** `lib/round/presentation/components/skins_row.dart` — scorecard row showing skin winner initials per hole
...
- **Create:** `lib/round/presentation/components/handicap_row.dart` — scorecard row showing net scores
...
1. `flutter run` — from a completed round detail, scorecard displays with all players' scores and golf indicators (circles for birdies, squares for bogeys, etc.). Skins row shows winner initials or dashes. Handicap row shows net scores.
...
2. `flutter test --coverage` — ≥95% line coverage
